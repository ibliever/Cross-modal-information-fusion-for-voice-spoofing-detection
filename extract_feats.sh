...
if [ $stage -le 3 ]; then
   ...
   for name in LA_train LA_dev LA_eval; do
       for feat_type in cqt; do
           ...
           python3 feats_extraction/convertID2index.py --scp-file data/${feat_type}/${name}/feats_slicing.scp --sysID-file data/${name}/utt2systemID --out-file data/${feat_type}/${name}/utt2index --access-type ${name:0:2} || exit 1
       done
   done
   exit 0
fi
