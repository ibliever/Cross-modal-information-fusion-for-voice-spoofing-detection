...
def main(pretrained, data_files, model_params, training_params, device):
    ...
    score_file_pth = os.path.join(data_files['scoring_dir'], str(pretrained_id) + '-epoch%s-train_scores.txt' %(epoch_id))
    print("===> train scoring file saved at: '{}'".format(score_file_pth))
    prediction.prediction(train_loader, model, device, score_file_pth, data_files['train_utt2systemID'])
