...
import sys
import os
import data_utils_fuse as data_utils
import numpy as np
...
import models_fuse as models 
from models_fuse import SpectrogramModel, MFCCModel, CQCCModel, DenseNet, Bottleneck, CQCCBotNet, SpectBotNet, MFCCBotNet, LFCCBotNet, FeatureExtractor, deit_base_patch16_224, seDenseTransNet, SEDenseNet
...
def train_epoch(data_loader, model, lr, device):
    ...
    optim = optimizer.ScheduledOptim(
            ...
            betas=(0.9, 0.98), eps=1e-09, weight_decay=1e-4, lr=3e-4, amsgrad=True), 1000)
...
if __name__ == '__main__':
    ...
    track = args.track
    assert args.features in ['fuse_cqt_seres2net', 'fuse_cqt_sedense', 'mfcc', 'spect', 'cqcc', 'mfcc_dense', 'spect_dense', 'cqcc_dense', 'lfcc', 'lfcc_dense', 'lfcc_sedense', 'mfcc_botnet', 'spect_botnet', 'lfcc_botnet', 'cqcc_botnet', 'lfcc_deit', 'cqcc_deit', 'mfcc_deit', 'spect_deit', 'lfcc_sedensetrans', 'fuse_sedense', 'fuse_spect_sedense'], 'Not supported feature'
    ...
    elif args.features == 'fuse_spect_sedense':
        feature_fn = None
        model_cls = SEDenseNet
        print("*************fuse_spect_seDenseNetModel**********************")
    elif args.features == 'fuse_cqt_sedense':
                feature_fn = None
                model_cls = SEDenseNet
                print("*************fuse_cqt_seDenseNetModel**********************")
    elif args.features == 'fuse_cqt_seres2net':
                feature_fn = None
                model_cls = models.se_res2net50_v1b_26w_8s()
                print("*************fuse_cqt_seDenseNetModel**********************")
